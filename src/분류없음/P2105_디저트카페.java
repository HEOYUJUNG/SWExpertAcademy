		...
		for (int tc = 1; tc <= T; tc++) {
			...
			for (int i = 0; i < N - 2; i++) {
				for (int j = 0; j < N - 2; j++) {
					...
					startR = i; // 시작 지점 저장
					startC = j;
//					System.out.println("cafeTour i : " + i + " / j : " + j);
	...
	public static void cafeTour(int cnt, int r, int c, int dir, boolean change) {
		...
		if (r == startR && c == startC && cnt > 0 && path.size() > 1) {
			max = (max < cnt) ? cnt : max;
			return;
		}

		// 원점으로 돌아온건 아니지만 종료해줘야 하는 조건들
		// 범위 벗어나거나 or 방문한 적 있거나 or 디저트 중복거나 or 진행중인 방향이 아닌데 간 적 있는 방향이면
		if (dir != -1) {
...
//			if (check(r + dr[d], c + dc[d]) && !visited[r + dr[d]][c + dc[d]]) {
			// 다음 카페 가기
			if (dir == d) {
