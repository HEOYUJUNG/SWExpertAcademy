// 이거 답은 나오는데 시간초과!!
// 링크드 리스트의 이점을 살리지 못하고 불필요한 연산이 많다.
// 서로 연결해주려고 parent노드나 child 노드를 중복되게 많이 저장해야 하기 때문에 메모리, 시간 많이 썼을 것임
// 트리 관련 문제 나오면 배열로 접근하는 것이 나을 듯!!
// Node 구현해서 링크드 리스트 쓰고싶다면 링크드 리스트를 직접 구현해서 써라!!
