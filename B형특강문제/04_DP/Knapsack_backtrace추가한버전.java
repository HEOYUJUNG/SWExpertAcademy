...
public class Knapsack_backtrace추가한버전 {
	public static void main(String[] args) throws Exception {
		BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
		StringTokenizer st;
		StringBuilder sb = new StringBuilder();
		int[][] dp = new int[101][1001];
		int[][] come = new int[101][1001]; // 1: r번째 물건 넣지 않은 경우, 2 : r번째 물건 넣은 경우
		int[] w = new int[101];
		int[] v = new int[101];
		...
		for (int tc = 1; tc <= T; tc++) {
			...
			// 가치의 합이 최대가 되도록 가방에 들어있는 물건 추적하기
			boolean[] used = new boolean[N + 1];
			int c = V; // 현재 가방 부피
			for (int r = N; r >= 1; r--) {
				if (come[r][c] == 2) { // r번째 물건 넣은 경우
					used[r] = true;
					c -= w[r];
				}
			}
